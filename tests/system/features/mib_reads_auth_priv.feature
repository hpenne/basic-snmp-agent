Feature: SNMP MIB reads with authPriv security over plain TCP
  ...
  @REQ-0074 @REQ-0075 @REQ-0078 @REQ-0079 @REQ-0086 @REQ-0088 @REQ-0091 @REQ-0101 @REQ-0107
  Scenario: GET with correct authPriv credentials succeeds
    When snmpget at authPriv with user "privuser", auth password "authpassword", and priv password "authpassword" queries OID "1.3.6.1.2.1.1.1.0" from the agent
  ...
  Scenario: GETNEXT with correct authPriv credentials succeeds
    When snmpgetnext at authPriv with user "privuser", auth password "authpassword", and priv password "authpassword" queries OID "1.3.6.1.2.1.1.1.0" from the agent
  ...
  Scenario: GETBULK with correct authPriv credentials succeeds
    When snmpbulkget at authPriv with user "privuser", auth password "authpassword", and priv password "authpassword", non-repeaters=0 and max-repetitions=2 queries OID "1.3.6.1.2.1.1.1.0" from the agent
    Then the SNMP response contains OID "1.3.6.1.2.1.1.3.0" with string value "Timeticks: (0) 0:00:00.00"
    And the SNMP response contains OID "1.3.6.1.2.1.1.5.0" with string value "test-agent-mib-auth-priv"

  @REQ-0080 @REQ-0093 @REQ-0094 @REQ-0098 @REQ-0099 @REQ-0101 @REQ-0107
  Scenario: GET without explicit engine ID succeeds after automatic discovery with authPriv
    When snmpget at authPriv without explicit engine ID with user "privuser", auth password "authpassword", and priv password "authpassword" queries OID "1.3.6.1.2.1.1.1.0" from the agent
    Then the SNMP response contains OID "1.3.6.1.2.1.1.1.0" with string value "basic-snmp-agent test instance"

  @REQ-0100
  Scenario: GET with wrong authentication password is rejected
    When snmpget at authPriv with user "privuser", auth password "wrongpassword", and priv password "authpassword" queries OID "1.3.6.1.2.1.1.1.0" from the agent
  ...
  Scenario: GET with unknown user name is rejected
    When snmpget at authPriv with user "nobody", auth password "authpassword", and priv password "authpassword" queries OID "1.3.6.1.2.1.1.1.0" from the agent
    Then the SNMP request times out or returns an error
  ...
  Scenario: GET with correct authPriv credentials but wrong contextEngineID receives Report PDU
    When snmpget at authPriv with wrong context engine ID "0x80001f8804776f726f6e67" with user "privuser", auth password "authpassword", and priv password "authpassword" queries OID "1.3.6.1.2.1.1.1.0" from the agent
