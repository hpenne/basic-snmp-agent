Feature: SNMP MIB reads over plain TCP
  ...
  Background:
    Given a test-agent-mib instance is running with engine ID "0x800001f88404746573742d6167656e742d6d6962"

  @REQ-0021 @REQ-0022 @REQ-0023 @REQ-0068 @REQ-0069 @REQ-0070 @REQ-0071 @REQ-0073
  ...
  Scenario: GETBULK with max-repetitions=0 returns only non-repeater varbinds
    ...
    Then the SNMP response contains OID "1.3.6.1.2.1.1.1.0" with string value "basic-snmp-agent test instance"
    And the SNMP response does not contain OID "1.3.6.1.2.1.1.3.0"

  @REQ-0021 @REQ-0022 @REQ-0026 @REQ-0068 @REQ-0073
  Scenario: GETBULK returns multiple values with max-repetitions=2
    ...
    Then the SNMP response contains OID "1.3.6.1.2.1.1.1.0" with string value "basic-snmp-agent test instance"
    And the SNMP response contains OID "1.3.6.1.2.1.1.3.0"

  @REQ-0057
  Scenario: Request with wrong engine ID is silently discarded
    When snmpget with wrong engine ID "0x800001f88404776f726f6e67" queries OID "1.3.6.1.2.1.1.1.0" from the agent
