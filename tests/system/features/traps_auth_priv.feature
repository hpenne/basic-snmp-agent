Feature: Authenticated and encrypted SNMP trap sending at authPriv security level
  ...
  Scenario: Cold-start trap at authPriv includes mandatory RFC 3416 varbinds
    When the agent at authPriv with user "privtrapuser", auth password "authpassword", and priv password "privpassword" sends a trap with OID "1.3.6.1.6.3.1.1.5.1"
  ...
  Scenario: Trap with Integer32 varbinds at authPriv preserves values in transit
    When the agent at authPriv with user "privtrapuser", auth password "authpassword", and priv password "privpassword" sends a trap with OID "1.3.6.1.6.3.1.1.5.3" and varbinds:
      | oid                    | type      | value |
  ...
  Scenario: Authenticated and encrypted trap at authPriv is delivered to multiple destinations
    ...
    When the agent at authPriv with user "privtrapuser", auth password "authpassword", and priv password "privpassword" sends to receivers "snmptrapd" and "priv-receiver-2" a trap with OID "1.3.6.1.6.3.1.1.5.1"
  ...
  @REQ-0105
  Scenario: Trap with wrong privacy password is not received by snmptrapd
    When the agent at authPriv with user "privtrapuser", auth password "authpassword", and priv password "wrongprivpassword" sends a trap with OID "1.3.6.1.6.3.1.1.5.1"
